...
def load_and_preprocess_data():
    ...
    model_coverage = LassoCV(cv=10, random_state=42)
    model_coverage.fit(X, y_coverage)
    model_number = LassoCV(cv=10, random_state=42)
...
def predict():
    try:
        ...
        manual_pred_coverage = np.dot(X_new, model_coverage.coef_) + model_coverage.intercept_
        print(f"Manual prediction for coverage: {manual_pred_coverage}")
        
        # Manual prediction for number
        manual_pred_number = np.dot(X_new, model_number.coef_) + model_number.intercept_
